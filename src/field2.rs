...
/// One dimensional Field (Real in Spectral Space)
pub type Field1 = FieldBase<f64, f64, 1>;
/// One dimensional Field (Complex in Spectral Space)
pub type Field1Complex = FieldBase<f64, Complex<f64>, 1>;
/// Two dimensional Field (Real in Spectral Space)
pub type Field2 = FieldBase<f64, f64, 2>;
/// Two dimensional Field (Complex in Spectral Space)
...
pub trait Field<T1, T2, const N: usize> {
    ...
    fn grad(&self, deriv: [usize; 1], scale: Option<[T1; N]>) -> Array<T2, Dim<[usize; N]>>;
}
...
macro_rules! impl_field2 {
    ($a: ty) => {
            ...
            Complex<T>: ScalarOperand,
        {
            ...
            /// Gradient
            #[allow(clippy::cast_possible_wrap, clippy::cast_possible_truncation)]
            fn grad(&self, deriv: [usize; 1], scale: Option<[T; 2]>) -> Array2<Self::Output> {
                let mut output = self.space.differentiate(&self.vhat, deriv[0], 0);
                if let Some(s) = scale {
                    let scale_1: $a = (s[0].powi(deriv[0] as i32)).into();
