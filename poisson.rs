...
impl<const N: usize> Poisson<f64, N> {
    ...
    pub fn from_field(field: &FieldBase<f64, f64, f64, N>, c: [f64; N]) -> Self
    //where
    //    S: Spaced<f64, N>,
    {
        //Self::from_space(&field.space, c)
        let matvec: Vec<Option<MatVec<f64>>> = field
            .space
            .bases
            .iter()
    ...
    fn is_diag(base: &Base<f64>) -> bool {
        match base {
            ...
            Base::Fourier(_) | Base::FourierR2c(_) => true,
        }
    }
    ...
    fn matrix_from_base(base: &Base<f64>, c: f64) -> (Array2<f64>, Array2<f64>) {
        ...
        match base {
            ...
            Base::Fourier(_) | Base::FourierR2c(_) => (lap * c_t, mass),
        }
    }
    ...
    fn matvec_from_base(base: &Base<f64>) -> Option<MatVec<f64>> {
        ...
        match base {
            Base::Chebyshev(_) | Base::CompositeChebyshev(_) => Some(matvec),
            Base::Fourier(_) | Base::FourierR2c(_) => None,
        }
    }
}
